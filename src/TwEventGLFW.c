...
void TW_CDECL_CALL TwEventMouseButtonGLFW3(GLFWwindow* win,int glfwButton, int glfwAction,int mode)
{
    int handled = 0;
...
void TW_CDECL_CALL TwEventKeyGLFW3(GLFWwindow* win,int glfwKey, int scanCode,int action,int modes)
{
    int handled = 0;
...
void TW_CDECL_CALL TwEventCharGLFW3(GLFWwindow* win,unsigned int codePoint)
{
	// if( glfwAction==GLFW_PRESS && (codePoint & 0xff00)==0 )
    if((codePoint & 0xff00)==0 )
	{
           TwKeyPressed(codePoint, g_KMod);
	}
  //  return 0;
}

void TW_CDECL_CALL TwEventMousePosGLFW3(GLFWwindow* win,double mouseX, double mouseY){
	TwMouseMotion((int)mouseX,(int)mouseY);
}
void   TW_CDECL_CALL TwEventMouseWheelGLFW3(GLFWwindow* win,double axisX,double axisY){

	TwMouseWheel((int)axisY);//currently Y axis wheel scroll support
